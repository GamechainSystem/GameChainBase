...
class OperationTable extends BaseComponent {
    render() {
        let fee_row = this.props.fee.amount > 0 ? (
                <div >
                    <span className="gray" >{this.formatMessage('transfer_chargefee')}</span>
        ...
        trx.operations.forEach((op, opIndex) => {
            ...
            switch (ops[op[0]]) { // 遍历交易的操作类型
                case "transfer":
                    ...
                    rows.push(
                        <div key={key++} >
                            <span className="gray">{this.formatMessage('transfer_from')}</span>
                            <span>{this.linkToAccount(op[1].from)}</span>
                        </div>
                    );
                    rows.push(
                        <div key={key++} >
                            <span className="gray">{this.formatMessage('transfer_to')}</span>
...

                {
                    memo ?
                        rows.push(
                            <div key={key++} >
                                <span className="gray" >{this.formatMessage('transfer_memo')}</span>
                                {memo}
                            </div>
                        ) : null
                }
