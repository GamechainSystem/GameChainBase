...
class AssetSelector extends React.Component {
    ...
    componentWillReceiveProps(nextProps){
        this.setState({
            selected: nextProps.value||nextProps.assets[0]
        });
    }
    ...
    //    console.info("selected",this.props.value);
        ...
        if (this.props.assets.length == 1) {
            ...
            return (
                <select disabled={this.props.disabled} className="select" value={this.state.selected} onChange={this.onChange.bind(this)}>
                    {options}
                </select>
            );
        }
